...
module Pitchfork
  class TestConfigurator < Test
    TestStruct = Struct.new(
      *(Configurator::DEFAULTS.keys + %w(listener_opts listeners)))

    def test_config_init
      assert(Configurator.new {})
    end

    def test_expand_addr
      meth = Configurator.new.method(:expand_addr)
    ...
    def test_config_invalid
      ...
      assert_raises(NoMethodError) do
        Configurator.new(:config_file => tmp.path)
      end
    end
    ...
    def test_config_non_existent
      ...
      assert_raises(Errno::ENOENT) do
        Configurator.new(:config_file => path)
      end
    end

    def test_config_defaults
      cfg = Configurator.new(:use_defaults => true)
      test_struct = TestStruct.new
      cfg.commit!(test_struct)
      Configurator::DEFAULTS.each do |key,value|
        if value.nil?
    ...
    def test_listen_options
      ...
      cfg = Configurator.new(:config_file => tmp.path)
      test_struct = TestStruct.new
      cfg.commit!(test_struct)
    ...
    def test_listen_option_float_delay
      ...
      listener = "127.0.0.1:12345"
      tmp.syswrite("listen '#{listener}', #{expect.inspect}\n")
      assert Configurator.new(:config_file => tmp.path)
    end
    ...
    def test_check_client_connection
      ...
      # Nothing raised
      Configurator.new(:config_file => tmp.path).commit!(test_struct)
    ...
    def test_after_worker_fork_proc
      ...
      [ proc { |a,b| }, Proc.new { |a,b| }, lambda { |a,b| } ].each do |my_proc|
        Configurator.new(:after_worker_fork => my_proc).commit!(test_struct)
    ...
    def test_after_worker_fork_wrong_arity
      [ proc { |a| }, Proc.new { }, lambda { |a,b,c| } ].each do |my_proc|
        assert_raises(ArgumentError) do
          Configurator.new(:after_worker_fork => my_proc)
        end
      end
    end
  end
