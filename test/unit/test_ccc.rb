...
module Pitchfork
  class TestCccTCPI < Test
    def test_ccc_tcpi
      start_pid = $$
      host = '127.0.0.1'
      port = unused_port

      rd, wr = Info.keep_ios(IO.pipe)
      sleep_pipe = Info.keep_ios(IO.pipe)
      ready_read, ready_write = Info.keep_ios(IO.pipe)
