...
module Pitchfork
  class TestTeeInput < Test
    ...
    def check_tempfiles
      ...
      assert_instance_of TmpIO, tmp[0]
    end
    ...
    def test_big_body
      r = init_request('.' * Const::MAX_BODY << 'a')
      ti = TeeInput.new(@rd, r)
      assert_equal 0, @parser.content_length
      assert @parser.body_eof?
      assert_kind_of File, ti.tmp
      assert_equal 0, ti.tmp.pos
      assert_equal Const::MAX_BODY + 1, ti.size
      check_tempfiles
    end
    ...
    def test_big_body_multi
      r = init_request('.', Const::MAX_BODY + 1)
      ti = TeeInput.new(@rd, r)
      assert_equal Const::MAX_BODY, @parser.content_length
      assert ! @parser.body_eof?
      assert_kind_of File, ti.tmp
      assert_equal 0, ti.tmp.pos
      assert_equal Const::MAX_BODY + 1, ti.size
