...
class BrokenAppTest < Pitchfork::IntegrationTest
  def test_graceful_handling_of_broken_apps
    ...
    assert_instance_of(Net::HTTPInternalServerError, response.response)
    assert_includes File.read("stderr.log"), "app error: BAD (RuntimeError)"
    ...
    assert_equal 500, response.code.to_i
    assert_instance_of(Net::HTTPInternalServerError, response.response)
    assert_includes File.read("stderr.log"), "app error: \"undefined method `[]' for nil:NilClass"
